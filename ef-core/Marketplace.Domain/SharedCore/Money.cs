...
public class Money : ValueObject
{
  ...
  public static Money FromDecimal(decimal amount, string currencyCode,
    ICurrencyLookup currencyLookup) => new(amount, currencyCode,
      currencyLookup);

  public static Money FromString(string amount, string currencyCode,
    ICurrencyLookup currencyLookup) => new(Convert.ToDecimal(amount,
      new CultureInfo("en-US")), currencyCode, currencyLookup);
    ...
    ICurrencyLookup currencyLookup)
  {
    ...
    if (decimal.Round(amount, currency.DecimalPlaces) != amount)
    {
      ...
      message: $"Amount in {currencyCode} cannot have more than " +
